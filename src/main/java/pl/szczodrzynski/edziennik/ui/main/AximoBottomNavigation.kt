...
class AximoBottomNavigation @JvmOverloads constructor(
    ...
    fun refreshSettings() {
        ...
        menuItems.forEachIndexed { index, item ->
            val key = when (item.target) {
                NavTarget.GRADES -> "nav_grades"
                NavTarget.HOMEWORK -> "nav_homework"
                NavTarget.ATTENDANCE -> "nav_attendance"
                NavTarget.AGENDA -> "nav_timetable"
                NavTarget.NOTES -> "nav_messages"
                NavTarget.SETTINGS -> "nav_settings"
                else -> null
            }
            val enabled = key?.let { settingsPrefs.getBoolean(it, true) } ?: true
            menuViews[index].alpha = if (enabled) 1f else 0f
            menuViews[index].visibility = if (open && enabled) View.VISIBLE else View.INVISIBLE
        }
    }
    ...
    private fun openMenu() {
        ...
        open = true
        menuViews.forEachIndexed { i, view ->
            val key = when (menuItems[i].target) {
                NavTarget.GRADES -> "nav_grades"
                NavTarget.HOMEWORK -> "nav_homework"
                NavTarget.ATTENDANCE -> "nav_attendance"
                NavTarget.AGENDA -> "nav_timetable"
